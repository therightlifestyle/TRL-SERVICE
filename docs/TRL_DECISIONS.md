...
- **Decision:** Target Cloudflare Pages for hosting, DNS, and HTTPS for the intended domain therightlifestyle.com. _(Gate 6 note: the domain is not owned, and the build no longer assumes it — see D-021. The hosting direction is unchanged; the origin is now a build variable.)_
...
## D-019 — Phase 1 ships with no analytics

- **Date:** 2026-09-17
- **Status:** Founder decision (Gate 5 close), revisiting the open item carried since Gate 1
- **Decision:** No analytics provider, script, beacon, or server-side tracking in Phase 1. The site keeps zero client JavaScript (D-010), `script-src 'none'` on every static page (D-018), no cookies, and a privacy draft whose "no analytics or tracking of any kind" statement remains literally true.
- **Why:** The founder's Phase 1 question is "does the service work for real customers?", and the signals that answer it already arrive first-party: enquiry emails in the inbox (D-007), WhatsApp messages, and Cloudflare's free per-request metrics. Adding a provider would buy funnel data before there is a funnel, at the cost of loosening the CSP, narrowing D-010, expanding the privacy disclosure, and creating consent work — a real loss of a real guarantee in exchange for numbers nobody is yet positioned to act on. Cloudflare Web Analytics was considered and rejected on the same grounds: it is cookie-less and free, but it still injects a third-party script.
- **Consequence:** The CSP, the no-cookies posture, and the privacy draft need no change; the zero-JS baseline stays absolute rather than "except for analytics". `tests/unit/build-budget.test.ts` now pins zero client JavaScript as an artifact invariant, which makes the outcome enforceable rather than incidental. Revisit at Gate 7 or at the start of Phase 2, when the question changes from "whether to measure" to "what to measure". Traffic and conversion reporting is unavailable for Phase 1 by choice; `TRL_GATE5_REVIEW.md` records the trade.

## D-020 — Cache policy: specific rules only, and the two sides of the hashing line

- **Date:** 2026-09-17
- **Status:** Implementation decision (Gate 5 performance review)
- **Decision:** `public/_headers` sets `Cache-Control` on specific path patterns only: `/_astro/*` (hashed build output, `public, max-age=31536000, immutable`, injected by the adapter) and `/fonts/*` (`public, max-age=31536000, immutable`, authored). The catch-all `/*` rule deliberately sets none, leaving documents, `robots.txt`, the sitemap, and the favicon on the platform default `public, max-age=0, must-revalidate` with an ETag.
- **Why:** Everything on the wrong side of a `max-age=0` default is either revalidated needlessly or cached into a broken page, and the line is content-hashing. Documents are not content-hashed while the stylesheets they reference are, so a stale document would point at a stylesheet that no longer exists and render unstyled — revalidation is correct there. The 48 KB of self-hosted fonts are render-critical on every route and effectively immutable, and were measured revalidating on every visit before this decision.
- **Why not pin the catch-all:** measured, not assumed. `_headers` rules are merged rather than overridden, so a `Cache-Control` on both `/*` and `/_astro/*` produced one field carrying two `max-age` directives (`public, max-age=0, must-revalidate, public, max-age=31536000, immutable`), which RFC 9111 says a cache must treat as stale — silently defeating the specific rule. It also suppressed the adapter's injected `/_astro/*` rule, which the adapter skips whenever an existing rule already sets `Cache-Control` on a match. Setting `Cache-Control` on the catch-all is therefore prohibited in this repository.
- **Consequence:** `immutable` on `/fonts/*` is only safe while the bytes behind those URLs never change, and the filenames are not content-hashed — so the deployment contract is that a font change ships under a new filename (family, weight, and subset are already encoded in the name). `tests/unit/security-headers.test.ts` pins all three rules and the one-Cache-Control-per-response constraint, `tests/unit/build-budget.test.ts` pins the byte budgets, and the reasoning is written into `public/_headers` so the catch-all experiment is not repeated.

## D-021 — The production origin is a required build variable, and no domain is asserted as a fact

- **Date:** 2026-09-19
- **Status:** Founder decision (Gate 6), revisiting the origin handling in D-006
- **Decision:** `PUBLIC_SITE_URL` is read from the environment by `astro.config.mjs`, is **required for any build**, and has no default. A build without it fails, before writing any output, with a message naming the variable and the command to fix it. `therightlifestyle.com` is removed from `src/lib/site.ts` and from `public/robots.txt`; it remains *intended* and unowned, recorded in `TRL_MASTER_CONTEXT.md` as an intention rather than an approved fact.
- **Why:** The build previously defaulted to `https://therightlifestyle.com`, which meant every build asserted a canonical origin, an Open Graph URL, an Organization `url`, a sitemap, and a `robots.txt` sitemap pointer the business cannot back — and, on any deployment that was not that domain, silently produced the wrong origin in all of them. The founder does not own the domain, so there is no honest default to fall back to: the choice is a wrong value or an explicit one, and only one of those fails loudly. This is the same rule the rest of the project already follows for prices, proof, and credentials — do not present what you cannot back.
- **Why it is a build failure and not a warning:** A missing origin produces a site that mostly works and is quietly wrong about its own identity, everywhere, in a way nobody notices until search engines or link previews disagree. A failed build with a two-line fix is strictly cheaper.
- **Consequence:** `astro:build:start` carries a `trl-deployment-guard` integration that validates presence, scheme, and origin-only shape; `astro dev` is unaffected (no `site` configured means the request's own origin is used). `src/layouts/BaseLayout.astro` derives canonical, OG, and JSON-LD URLs from `Astro.site`; `src/pages/robots.txt.ts` generates `robots.txt` from the same value, so the committed file carrying a hardcoded domain is gone. Tests no longer assert a domain: they read the origin from the built artifact (`tests/unit/helpers/build-artifacts.ts`) and assert that everything agrees with it. CI sets `PUBLIC_SITE_URL=https://ci.invalid` — a reserved, non-resolvable name — because a CI build is a throwaway artifact and must not claim a real host.

## D-022 — The interim deployment ships noindex, and indexing is one explicit variable

- **Date:** 2026-09-19
- **Status:** Founder decision (Gate 6)
- **Decision:** Indexing is **off unless the build is explicitly told otherwise**. `PUBLIC_ALLOW_INDEXING` is read through `import.meta.env` (so it works as a real environment variable on the platform and from `.env` locally); only the exact string `true` turns indexing on, and anything else — unset, `false`, a typo, `yes`, `1` — means off. While it is off, every page carries `<meta name="robots" content="noindex, follow">` and `robots.txt` does not advertise the sitemap. `npm run preflight` warns if indexing is on while the origin is still a `workers.dev` host.
- **Why the default is off:** The first deployment is a temporary host. An indexed interim host competes with the site's own future domain in search results, and undoing that takes weeks rather than minutes. A default that fails towards invisibility is the only safe direction; the exposure is one variable away when the real domain exists, and it cannot be forgotten in the other direction because the missing variable is the quiet one.
- **Why crawling is allowed while indexing is off (the counter-intuitive part):** `robots.txt` governs crawling, not indexing, so the obvious `Disallow: /` is the wrong tool here. Blocking the crawl would stop crawlers from ever fetching a page and therefore from ever reading the `noindex` directive on it — and a URL discovered through an external link can still be listed as *"indexed, though blocked by robots.txt"*. `Noindex:` inside `robots.txt` is not a directive either; Google removed support for it on 2019-09-01. The page-level `noindex` is the mechanism that does the work, and `robots.txt` stays out of its way. The policy is generated from the same two facts the pages use, so the two cannot contradict each other.
- **Consequence:** `src/lib/indexing.ts` is the single reader and holds the pure policy; `src/layouts/BaseLayout.astro` applies it to every page while preserving the permanent `noindex` on the legal drafts, the confirmation page, and the 404 document; `public/robots.txt` is deleted in favour of the generated `src/pages/robots.txt.ts`. `tests/unit/robots-policy.test.ts` asserts both states, including the trap above. The sitemap is still built — it is simply not advertised while indexing is off, and the pages it lists say `noindex` — which keeps the build identical in both states apart from the two intended differences.

## D-023 — No `account_id` or account-scoped identifier is committed

- **Date:** 2026-09-19
- **Status:** Implementation decision (Gate 6)
- **Decision:** `wrangler.jsonc` carries no `account_id`. The `ratelimits` block stays commented until the founder's account-scoped `namespace_id` exists, and `npm run preflight` fails if either a real `account_id` or an unfilled placeholder appears in the configuration.
- **Why:** `account_id` is optional for Wrangler, and Cloudflare's own troubleshooting guidance for an incorrect value is to *remove* it (`Incorrect account_id` / error 7003) because a wrong integer deploys against whichever account actually owns that identifier. Workers Builds supplies the account context to its builds — the API token it creates automatically includes account settings read — so the founder's deployment path needs nothing committed. Committing a guessed value would fabricate infrastructure and could deploy into an account that is not the founder's.
- **Why it is asserted and not just documented:** An invented `account_id` is exactly the kind of value that looks harmless in review and is discovered in production. `tests/unit/deployment-config.test.ts` fails if one appears in the source or in the generated deploy config, and `npm run preflight` fails too.
- **Consequence:** A deploy from a laptop or a different CI must pass the account explicitly (`CLOUDFLARE_ACCOUNT_ID=... CLOUDFLARE_API_TOKEN=... npx wrangler deploy`), which is documented in `TRL_DEPLOYMENT.md`. The generated deploy config is validated on every pull request by `npx wrangler deploy --dry-run`, which needs no account: a broken deploy configuration now fails CI rather than the founder's first deploy.
