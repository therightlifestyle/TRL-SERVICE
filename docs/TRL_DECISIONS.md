...
## D-021 — Gate 6 deployment-readiness procedures: split repository-completable from founder-only

- **Date:** 2026-09-17
- **Status:** Implementation decision (Gate 6 close)
- **Decision:** The deployment-gate checklist is now procedure in `TRL_DEPLOYMENT.md`, with an explicit split at the top between work that can be closed from inside the repository (this session's work) and work that only the founder can do (every remaining step). Concretely:
  - **Repository-completable (closed at Gate 6):** environment-variable inventory, production deployment procedure, preview verification procedure, smoke-test procedure, rollback procedure, recovery procedure, backup/repository-recovery guidance, domain/DNS readiness checklist, Cloudflare deployment checklist, Turnstile production-key checklist, Resend production-email checklist, rate-limit namespace configuration checklist, final platform security checklist (framing protection + HSTS), GitHub Pages / Jekyll transition procedure, and the `tests/unit/wrangler-config.test.ts` artifact pin (4 tests).
  - **Founder-only (cannot be closed from the repository):** every real account, every real DNS change, every real secret, the rate-limit `namespace_id`, the framing/HSTS platform rules on the final HTTPS domain, disabling GitHub Pages (the repository token gets `403` on the Pages admin endpoint), one real enquiry end-to-end, and the rate-limit burst check on the deployed origin.
- **Why:** the same reason the no-deploy constraint was carried as D-004 — accounts, DNS, and credentials are not the agent's to create, and Gate 6 was deliberately defined as the gate that turns a checklist into procedure without inventing any of the things the founder owns. A procedure that did not record what only the founder can do would either be a fake completion or it would invite the agent to do work it should not.
- **Consequence:** `TRL_DEPLOYMENT.md` opens with a checked "repository-completable" list and an unchecked "founder-only" list, so the next session — and the next reviewer — can see the gate's two halves at a glance. `tests/unit/wrangler-config.test.ts` makes the deploy-config artifact honest: the `ratelimits` block remains a comment with a placeholder, and no fabricated `namespace_id` (or any quoted digit string in that key) can be committed without failing CI. The founder's `namespace_id`, when chosen, lands in `wrangler.jsonc` as a one-line edit; the endpoint picks the binding up automatically — no code change. No secrets are introduced anywhere in this gate.
